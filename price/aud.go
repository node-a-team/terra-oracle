...
func (ps *PriceService) audToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			// resp, err := http.Get("https://quotation-api-cdn.dunamu.com/v1/forex/recent?codes=FRX.KRWAUD")
			resp, err := http.Get(cfg.Config.APIs.AUD.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent aud/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("aud/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
