...
func (ps *PriceService) cadToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			// resp, err := http.Get("https://quotation-api-cdn.dunamu.com/v1/forex/recent?codes=FRX.KRWCAD")
			resp, err := http.Get(cfg.Config.APIs.CAD.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent cad/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("cad/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
