...
func (ps *PriceService) inrToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			// resp, err := http.Get("https://quotation-api-cdn.dunamu.com/v1/forex/recent?codes=FRX.KRWINR")
			resp, err := http.Get(cfg.Config.APIs.INR.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent inr/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("inr/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
