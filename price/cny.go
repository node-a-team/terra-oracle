...
func (ps *PriceService) cnyToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			// resp, err := http.Get("https://quotation-api-cdn.dunamu.com/v1/forex/recent?codes=FRX.KRWCNY")
			resp, err := http.Get(cfg.Config.APIs.CNY.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent cny/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("cny/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
