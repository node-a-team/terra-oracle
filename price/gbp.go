...
func (ps *PriceService) gbpToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			// resp, err := http.Get("https://quotation-api-cdn.dunamu.com/v1/forex/recent?codes=FRX.KRWGBP")
			resp, err := http.Get(cfg.Config.APIs.GBP.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent gbp/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("gbp/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
