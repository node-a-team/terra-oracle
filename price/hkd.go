...
func (ps *PriceService) hkdToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			// resp, err := http.Get("https://quotation-api-cdn.dunamu.com/v1/forex/recent?codes=FRX.KRWHKD")
			resp, err := http.Get(cfg.Config.APIs.HKD.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent hkd/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("hkd/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
