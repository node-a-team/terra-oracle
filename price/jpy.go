...
func (ps *PriceService) jpyToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			resp, err := http.Get(cfg.Config.APIs.JPY.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent jpy/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("jpy/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
