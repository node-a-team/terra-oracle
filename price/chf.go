...
func (ps *PriceService) chfToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			// resp, err := http.Get("https://quotation-api-cdn.dunamu.com/v1/forex/recent?codes=FRX.KRWCHF")
			resp, err := http.Get(cfg.Config.APIs.CHF.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent chf/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("chf/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
