...
func (ps *PriceService) sgdToKrw(logger log.Logger) {
	for {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			// resp, err := http.Get("https://quotation-api-cdn.dunamu.com/v1/forex/recent?codes=FRX.KRWSGD")
			resp, err := http.Get(cfg.Config.APIs.SGD.Dunamu)
			if err != nil {
			...
			logger.Info(fmt.Sprintf("Recent sgd/krw: %s, timestamp: %d", price, timestamp))
			...
			ps.SetPrice("sgd/krw", sdk.NewDecCoinFromDec("krw", decAmount), timestamp)
		}()
	}
}
